...
namespace Salamzadeh\PBPayment\Gateways\Sadad;
...
use Salamzadeh\PBPayment\Http\CurlRequest;
...
class Sadad extends AbstractGateway implements GatewayInterface
{
    ...
    public function initialize(array $parameters = []): self
    {
        ...
        $this->setMerchantId(
            $parameters['merchant_id'] ?? app('config')->get('pbpayment.sadad.merchant_id')
        );
        $this->setTerminalId(
            $parameters['terminal_id'] ?? app('config')->get('pbpayment.sadad.terminal_id')
        );
        $this->setTerminalKey(
            $parameters['terminal_key'] ?? app('config')->get('pbpayment.sadad.terminal_key')
        );

        $this->setAppName($parameters['app_name'] ?? app('config')->get('pbpayment.sadad.app_name'));
        $this->setLocalDateTime($parameters['local_date_time'] ?? Carbon::now());

        $this->setCallbackUrl($parameters['callback_url']
            ?? app('config')->get('pbpayment.sadad.callback-url')
    ...
    protected function purchaseViewParams(): array
    {
        return [
            ...
            'image' => 'https://raw.githubusercontent.com/salamzadeh/pb-payment/master/resources/assets/img/sadad.png',
