...
trait TransactionData
{
     ...
     *
     * @var PBPaymentTransaction|null
     */
    protected ?PBPaymentTransaction $transaction = null;
     ...
     *
     * @param PBPaymentTransaction|Model $transaction
     * @return $this
     */
    public function setTransaction(PBPaymentTransaction $transaction): self
    {
     ...
     *
     * @return PBPaymentTransaction|null
     */
    public function getTransaction(): ?PBPaymentTransaction
    {
    ...
    {
        if (is_numeric($uid)) {
            $transaction = PBPaymentTransaction::find($uid);
        } elseif (is_string($uid)) {
            $transaction = PBPaymentTransaction::where('code', $uid)->first();
        }
    ...
    {
        $transactions = PBPaymentTransaction::where('payable_id', $payable_id);
    ...
    {
        $transaction = new PBPaymentTransaction(array_merge(
            [
        ...
        $transaction->status = PBPaymentTransaction::T_INIT;
        $transaction->code = Str::random(app('config')->get('pbpayment.code_length' ,16));
    ...
    protected function transactionSucceed(array $params = []): void
    {
        ...
        $this->transaction->status = PBPaymentTransaction::T_SUCCEED;
        $this->transaction->save();
    }

    protected function transactionFailed(string $errors = null): void
    {
        $this->transaction->status = PBPaymentTransaction::T_FAILED;
    ...
    {
        $this->transaction->fill($params);
        $this->transaction->status = PBPaymentTransaction::T_PENDING;
        $this->transaction->save();
    }
    ...
    {
        $this->transaction->fill($params);
        $this->transaction->status = PBPaymentTransaction::T_VERIFY_PENDING;
        $this->transaction->save();
    }
    ...
    {
        $this->transaction->fill($params);
        $this->transaction->status = PBPaymentTransaction::T_PAID_BACK;
        $this->transaction->save();
    }
}
